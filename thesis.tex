...
%ZAK: is "useful invariant" the right phrasing? Maybe it's more correct to
%ZAK: consider whether HPCs are an invariant at all
...
%ZAK: I would delete this "most" since we already had "mostly" earlier
...
following question:
%ZAK: this first point is worded somewhat awkwardly, maybe consider splitting
%ZAK: the sentence?
...
%ZAK: don't undersell yourself, you developed a module based on the API
...
%ZAK: maybe ignore that, I would put it at low priority.
%ZAK: If you do want to use something, maybe this?
%ZAK: http://www.zdnet.com/article/virtualizing-the-enterprise-an-overview/
...
%ZAK: Is it possible to give a brief summary of what technique the arehardware
%ZAK: uses to analyze the counters? Update: maybe okay since you talk more about
%ZAK: it later
...
%ZAK: is "has proved" the correct wording? Maybe just "are"?
...
%ZAK: might help to define overcount
...
%ZAK: what about false negatives?
...
%ZAK: Sorry for the confusion, the "do we do this" was included because it reads
%ZAK: "best interpreted," which may bring up the "if that is the best way, why
%ZAK: don't we do all of these techniques?" question
...
%ZAK: which project? That reminds me, we now have a UIUC gitlab, probably the 
%ZAK: thing for us to do is push all your stuff over there
...
%ZAK: "transfers back?" sorry, this is confusing...
...
%ZAK: might want to describe more what you mean by entropy
...
%ZAK: black-box because we do not directly inform that hypervisor of what is
%ZAK: running above it?
...
%ZAK: add the red bar graphs if they aren't already?
...
%ZAK: might be more clear to say both kprof and the modified QEMU?
...
%ZAK: other areas of the stack (e.g., ...
possible to combine data from other areas of the stack with these HPCs to build
%ZAK: maybe say "basic blocks of the complied software"
...
%ZAK: hypervisor state in terms of...? And by data I'm assuming HPC data
...
%ZAK: Just to add on what we discussed, we will also send them kprof since I
%ZAK: think they will use it
...
%ZAK: Maybe mention the distinguishability of Linux and Windows in your
%ZAK: experiments again?
